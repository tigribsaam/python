...
def schedule():
    ...
    for gym_class in gym_day.get_gymclasses():
        ...
        class08_label.grid(row=1, column= gym_day.get_gymclasses().index(gym_class))
        instructor08_label = Label(schedule_box, text='Instructor: - ')
        instructor08_label.grid(row=2, column= gym_day.get_gymclasses().index(gym_class))
        members08_listbox = Listbox(schedule_box)
        members08_listbox.grid(row=3, column= gym_day.get_gymclasses().index(gym_class))
...
week_back_btn = Button(navigation_box, text="<< Vorige week", command=lambda: change_day(-7)).grid(row=4, column=0, padx= 10)
day_back_btn = Button(navigation_box, text="< Gisteren", command=lambda: change_day(-1)).grid(row=4, column=1, padx= 10)
day_forward_btn = Button(navigation_box, text="Morgen >", command=lambda: change_day(1)).grid(row=4, column=2, padx= 10)
week_forward_btn = Button(navigation_box, text="Volgende weerk >>", command=lambda: change_day(7)).grid(row=4, column=3, padx= 10)
